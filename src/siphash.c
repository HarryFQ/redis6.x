/*
   ...
   1. We use SipHash 1-2. This is not believed to be as strong as the
      ...
      我们用 SipHash算法 1-2。这被认为是不强大的建议2-4变种，但恐怕不是微不足道的攻击对抗这种减速器，它以同样的速度运行就像我们之前使用的Murmurhash2，为什么2-4变慢了Redis下降了4%左右。

   2. Hard-code rounds in the hope the compiler can optimize it more
      in this raw from. Anyway we always want the standard 2-4 variant.
      (2. 硬编码轮询，希望编译器能进一步优化它在这原始的。无论如何，我们总是想要标准的2-4版本。)
   ...
   3. Modify the prototype and implementation so that the function directly
      ...
      (3.修改原型和实现，使功能直接实现返回一个uint64_t值，即哈希本身，而不是接收输出缓冲区。这也意味着输出大小被设置为8字节并且删除了16字节的输出代码处理。)
   4. Provide a case insensitive variant to be used when hashing strings that
      ...
      (4. 提供不区分大小写的变体，以便在对字符串进行散列时使用必须被哈希表认为是相同的，无论在哪种情况下。如果我们没有直接的大小写不敏感哈希函数，我们需要这样做在一些临时缓冲区中执行文本转换，这是昂贵的。)
   ...
   6. Modified the original test.c file to be a stand-alone function testing
     ...
     （6. 将原来的test.c文件修改为独立的函数测试函数的新形式(重新生成一个uint64_t)相关的测试向量。）
 ...
 * （类似的快速tolower()函数，它不关心语言环境，但只返回a-z的insetad）
 * */
int siptlw(int c) {
    if (c >= 'A' && c <= 'Z') {
        return c + ('a' - 'A');
